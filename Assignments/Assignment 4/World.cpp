...
void World::loadNodes (const string& filename) {
	...
	if (!read_data) {
		...
		unsigned int tempLoadNodes;
		read_data >> node_count;
		// read in the 4 numbers
		for (int i = 0; i < node_count; i++) {
				read_data >> tempLoadNodes;
				// read in the description numbers (maybe with a temp variable) north, south, east, west
				nodeArray[i] = tempLoadNodes;
		}
	}
